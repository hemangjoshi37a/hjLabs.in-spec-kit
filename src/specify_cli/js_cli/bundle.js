  ...
  "dist/cli/InitProjectCommand.js"(exports2) {
    ...
    var InitProjectCommand = class {
      ...
      async createBasicStructure(targetDir, projectName, options) {
        ...
        await fs2.ensureDir(path2.join(specifyDir, "templates"));
        await fs2.ensureDir(path2.join(specifyDir, "memory"));
...
`;
      ...
      async setupAIToolCommands(targetDir, aiModel) {
        const currentRepoRoot = this.findSpecKitRoot();
        if (path2.resolve(targetDir) !== path2.resolve(currentRepoRoot)) {
          await this.copySpecifyAssets(targetDir, currentRepoRoot);
        } else {
          console.log(chalk_12.default.yellow("\u{1F4CD} Initializing in spec-kit repo, skipping asset copy"));
        }
        switch (aiModel) {
          case "claude":
          ...
          case "copilot":
            ...
            break;
          default:
            console.log(chalk_12.default.yellow(`\u26A0\uFE0F Unknown AI model: ${aiModel}, setting up Claude commands as default`));
            await this.setupClaudeCommands(targetDir, currentRepoRoot);
        }
      }
      findSpecKitRoot() {
        ...
        while (currentDir !== path2.dirname(currentDir)) {
          const specifyDir = path2.join(currentDir, ".specify");
          const packageSpecifyDir = path2.join(currentDir, "specify_cli", ".specify");
          if (fs2.existsSync(specifyDir)) {
            return currentDir;
          }
          if (fs2.existsSync(packageSpecifyDir)) {
            return path2.join(currentDir, "specify_cli");
          }
          currentDir = path2.dirname(currentDir);
        }
        if (fs2.existsSync(path2.join(process.cwd(), ".specify"))) {
          return process.cwd();
        }
        try {
          const packagePath = require.resolve("../../package.json");
          const packageDir = path2.dirname(packagePath);
          if (fs2.existsSync(path2.join(packageDir, ".specify"))) {
            return packageDir;
          }
        } catch (e) {
        }
        console.log(chalk_12.default.yellow("\u26A0\uFE0F Could not find spec-kit templates, using current directory"));
        return process.cwd();
      }
      async copySpecifyAssets(targetDir, sourceRoot) {
        const sourceSpecifyDir = path2.join(sourceRoot, ".specify");
        const targetSpecifyDir = path2.join(targetDir, ".specify");
        const sourceTemplatesDir = path2.join(sourceSpecifyDir, "templates");
        const targetTemplatesDir = path2.join(targetSpecifyDir, "templates");
        if (await fs2.pathExists(sourceTemplatesDir)) {
          await fs2.copy(sourceTemplatesDir, targetTemplatesDir);
        }
        const sourceScriptsDir = path2.join(sourceSpecifyDir, "scripts");
        const targetScriptsDir = path2.join(targetSpecifyDir, "scripts");
        if (await fs2.pathExists(sourceScriptsDir)) {
          await fs2.copy(sourceScriptsDir, targetScriptsDir);
        }
        const sourceMemoryDir = path2.join(sourceSpecifyDir, "memory");
        const targetMemoryDir = path2.join(targetSpecifyDir, "memory");
        if (await fs2.pathExists(sourceMemoryDir)) {
          await fs2.copy(sourceMemoryDir, targetMemoryDir);
        }
      }
      async setupClaudeCommands(targetDir, sourceRoot) {
        const claudeDir = path2.join(targetDir, ".claude");
        const commandsDir = path2.join(claudeDir, "commands");
        await fs2.ensureDir(commandsDir);
        const sourceClaudeDir = path2.join(sourceRoot, ".claude");
        if (await fs2.pathExists(sourceClaudeDir) && path2.resolve(targetDir) !== path2.resolve(sourceRoot)) {
          await fs2.copy(sourceClaudeDir, claudeDir);
          console.log(chalk_12.default.green("\u2705 Claude slash commands configured"));
        } else if (path2.resolve(targetDir) === path2.resolve(sourceRoot)) {
          console.log(chalk_12.default.green("\u2705 Claude commands already exist in current directory"));
        } else {
          console.log(chalk_12.default.yellow("\u26A0\uFE0F No Claude commands found in source"));
        }
      }
      async setupGeminiCommands(targetDir, sourceRoot) {
        const geminiMdPath = path2.join(targetDir, "GEMINI.md");
...
`;
        await fs2.writeFile(geminiMdPath, geminiContent);
        console.log(chalk_12.default.green("\u2705 Gemini commands configured in GEMINI.md"));
      }
      async setupCopilotCommands(targetDir, sourceRoot) {
        const vscodeDir = path2.join(targetDir, ".vscode");
        ...
        await fs2.writeJSON(settingsPath, copilotConfig, { spaces: 2 });
        const copilotMdPath = path2.join(targetDir, "COPILOT.md");
...
`;
        await fs2.writeFile(copilotMdPath, copilotContent);
        console.log(chalk_12.default.green("\u2705 GitHub Copilot commands configured"));
      }
